 ...
 * options. The user may drag the "thumb" back and forth to choose the
 * selected option, or simply tap to toggle as if it were a checkbox.
 */
public class Switch extends CompoundButton {
    ...
    /**
     * Construct a new Switch with default styling, overriding specific
     * style attributes as requested.
     * 
     * @param context The Context that will determine this widget's
     *        theming.
     * @param attrs Specification of attributes that should deviate from
     *        default styling.
     */
    ...
    /**
     * Construct a new Switch with a default style determined by the given
    ...
    public Switch(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTextOn = res.getString(R.string.comment_header_hot);
        mTextOff = res.getString(R.string.comment_header_new);
    ...
    /**
     * Sets the switch text color, size, style, hint color, and highlight
     * color from the specified TextAppearance resource.
     */
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        final int maxTextWidth = Math.min(mSwitchTextMaxWidth, Math.max(mOnLayout.getWidth(), mOffLayout.getWidth()));
    ...
    private Layout makeLayout(CharSequence text, int maxWidth) {
        ...
        StaticLayout l = new StaticLayout(text, 0, text.length(), mTextPaint, actual_width, Layout.Alignment.ALIGN_NORMAL, 1.f, 0, true,
                TextUtils.TruncateAt.END, (int) Math.min(actual_width, maxWidth));
        return l;
    }
                ...
                switch (mTouchMode) {
                    ...
                    case TOUCH_MODE_DRAGGING: {
                        ...
                        if (newPos != mThumbPosition) {
                            ...
                            mTouchX = x;
                            invalidate();
                            if (mListener != null && getTargetCheckedState() != mState) {
                                Log.w("switch", "onTrSywitching");
                                mListener.onTrySwitching(getTargetCheckedState());
                            }
                        }
                        return true;
                    }
    ...
    private void animateThumbToCheckedState(boolean newCheckedState) {
        ...
        if (mState != newCheckedState) {
            Log.w("switch", "onSwitchChanged " + newCheckedState);
            mState = newCheckedState;
            if (mListener != null) {
                mListener.onSwitchChanged(mState);
            }
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.translate((thumbLeft + thumbRight) / 2 - switchText.getEllipsizedWidth() / 2, (switchInnerTop + switchInnerBottom) / 2 - switchText.getHeight()
                / 2);
